...
metadata: {"openclaw": {"emoji": "📈", "requires": {"url": "{{MCP_SERVER_URL}}"}, "primaryEnv": "MCP_SERVER_URL"}, "mode": "intranet"}
version: 1.0.0
---
...
- **OpenClaw** 已配置该 MCP 服务器为 **URL 连接**，例如：
  - `{{MCP_SERVER_URL}}`（将地址设为下方配置中的 url）
...
- 确认 OpenClaw 中配置的 MCP 服务器 URL 正确（即本 Skill 提供的 `{{MCP_SERVER_URL}}`）
- 确认内网机器上 MCP 已用 `--transport streamable-http` 启动且防火墙放行对应端口
...
在 OpenClaw 侧仅需添加「通过 URL 连接的 MCP 服务器」，将 **url** 设为下方地址即可。
